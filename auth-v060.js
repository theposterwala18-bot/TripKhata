...
(function(){
  ...
  function show(mode){
    ...
    d.innerHTML='<div style="width:min(410px,100%);background:white;border-radius:22px;padding:22px;box-shadow:0 20px 60px #0004"><div style="text-align:center"><div style="font-size:30px">🧳</div><div style="font-size:25px;font-weight:900;color:#153d76">TripKhata</div><div style="font-size:12px;color:#76839a">Friends • Trips • Hisab Simple</div></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;background:#f2f6fb;padding:4px;border-radius:12px;margin-top:18px"><button id="tkLoginTab">Login</button><button id="tkSignupTab">Create Account</button></div><input id="tkAuthName" placeholder="Your name" style="display:none;width:100%;box-sizing:border-box;margin-top:12px;padding:12px;border:1px solid #d7e0ec;border-radius:11px"><input id="tkAuthEmail" type="email" placeholder="Email address" style="width:100%;box-sizing:border-box;margin-top:10px;padding:12px;border:1px solid #d7e0ec;border-radius:11px"><input id="tkAuthPass" type="password" placeholder="Password (minimum 6 characters)" style="width:100%;box-sizing:border-box;margin-top:10px;padding:12px;border:1px solid #d7e0ec;border-radius:11px"><div id="tkAuthErr" style="display:none;margin-top:9px;padding:9px;background:#fff1f0;color:#b3261e;border-radius:9px;font-size:12px"></div><button id="tkAuthMain" style="width:100%;margin-top:13px;padding:13px;border:0;border-radius:11px;background:#1677ff;color:white;font-weight:900">Login</button><button id="tkGoogle" style="width:100%;margin-top:8px;padding:13px;border:1px solid #d7e0ec;border-radius:11px;background:white;font-weight:850">G&nbsp;&nbsp; Continue with Google</button><button id="tkPhoneBtn" style="width:100%;margin-top:8px;padding:13px;border:1px solid #d7e0ec;border-radius:11px;background:white;font-weight:850">📱 Continue with Mobile OTP</button><div id="tkPhonePanel" style="display:none;margin-top:10px;padding:10px;background:#f7f9fc;border-radius:11px"><input id="tkPhone" inputmode="tel" placeholder="+91 98765 43210" style="width:100%;box-sizing:border-box;padding:11px;border:1px solid #d7e0ec;border-radius:9px"><div id="tkRecaptcha" style="margin-top:8px"></div><button id="tkSendOtp" style="width:100%;margin-top:8px;padding:11px;border:0;border-radius:9px;background:#1677ff;color:#fff;font-weight:850">Send OTP</button><div id="tkOtpWrap" style="display:none"><input id="tkOtp" inputmode="numeric" placeholder="6-digit OTP" maxlength="6" style="width:100%;box-sizing:border-box;margin-top:8px;padding:11px;border:1px solid #d7e0ec;border-radius:9px"><button id="tkVerifyOtp" style="width:100%;margin-top:8px;padding:11px;border:0;border-radius:9px;background:#168a4b;color:#fff;font-weight:850">Verify OTP</button></div></div><div style="display:flex;justify-content:space-between;margin-top:11px"><button id="tkForgot" style="border:0;background:none;color:#1677ff">Forgot password?</button><button id="tkOffline" style="border:0;background:none;color:#6e7b91">Continue Offline</button></div></div>';
    ...
    let phoneConfirmation=null,recaptcha=null;
    tkPhoneBtn.onclick=()=>{tkPhonePanel.style.display=tkPhonePanel.style.display==='none'?'block':'none'};
    tkSendOtp.onclick=async()=>{try{
      tkAuthErr.style.display='none';
      let phone=tkPhone.value.replace(/\s+/g,'').trim();
      if(!phone)return errbox('Mobile number enter karo.');
    ...
    }catch(e){errbox(e.code==='auth/operation-not-allowed'?'Mobile OTP login Firebase Console vich enable nahi hai. Phone Authentication enable karo.':e.message)}
    };
    tkVerifyOtp.onclick=async()=>{try{
      const code=tkOtp.value.trim();if(!phoneConfirmation||code.length<6)return errbox('Valid OTP enter karo.');
